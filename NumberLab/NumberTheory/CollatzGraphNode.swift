...
class CollatzGraphNode: CustomStringConvertible {
    ...
    var prevList: [Odd]    // values of currently known immediate predecessors
                           // in Collatz map, sorted by increasing size
    
    init(value: Odd, prevList: [Odd] = []) throws {
        self.value = value
    ...
    var description: String {
        return "value: \(value.asInt()), next: \(next.asInt()), prevList: \(prevList.map { $0.asInt() })"
    }
    
    func addPrev(_ odd: Odd) throws {
        guard odd.collatzed() == value else { throw NumberError.collatzGraphError }
        let lower: Int = prevList.last?.asInt() ?? 0
        guard odd.asInt() > lower else { throw NumberError.collatzGraphError }
        prevList.append(odd)
    }

    func validatePrevList() -> Bool {
        var lower: Int = 0
...
extension CollatzGraphNode: Equatable {
    static func == (lhs: CollatzGraphNode, rhs: CollatzGraphNode) -> Bool {
        lhs.value == rhs.value && lhs.prevList == rhs.prevList
    }
}
...
//            print("key: \(key)")
        ...
        for key in keys {
            let prevList: [Odd] = try work[key]?.sorted().map { try Odd(n: $0) } ?? []
            let graphNode = try CollatzGraphNode(value: Odd(n: key), prevList: prevList)
//            print("graphNode: \(graphNode)")
    ...
    func serialize(to url: URL) throws {
        ...
        let json = CollatzGraphJSON(
            ...
            map: Dictionary(uniqueKeysWithValues: map.map { key, node in
                (String(key), node.prevList.map { $0.asInt() })
            })
        )
    ...
    static func deserialize(from url: URL) throws -> CollatzGraph {
        ...
        for (keyStr, prevInts) in json.map {
            ...
            let prevList = try prevInts.map { try Odd(n: $0) }
            let node = try CollatzGraphNode(value: try Odd(n: key), prevList: prevList)
            result[key] = node
        }
